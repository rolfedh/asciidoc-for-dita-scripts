...
2. **Create a release preparation branch:**
   ...
   ```sh
   git checkout -b release/v<new-version>  # Example: release/v0.1.9b3
   ```
...
6. **Create and push the release tag from main:**
   ...
   git checkout main
   git pull origin main
   git tag v<new-version>  # Example: v0.1.9b3
...
## Troubleshooting if the tagging gets ahead of the version
   ...
   git tag -d v0.1.9b3
   git tag v0.1.9b3
   git push --force origin v0.1.9b3
   ```
...
```sh
# Automated patch version bump (0.1.9b3 → 0.2.1)
make release
...
```sh
# Using GitHub CLI
gh release create v0.1.9b3 \
  --title "Release 0.1.9b3: Enhanced Plugin System" \
