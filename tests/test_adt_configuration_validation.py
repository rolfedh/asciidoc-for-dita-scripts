    ...
    try:
        ...
        for test_case in test_cases:
            ...
            try:
                ...
                print(f"    ✓ Skip comments: {getattr(module, 'skip_comments', 'NOT_SET')}")
                
                if test_case['should_succeed']:
                    results.append((test_case['name'], True, "Configuration accepted"))
                else:
                    results.append((test_case['name'], False, "Should have failed but didn't"))
        ...
        for test_case in test_contexts:
            ...
            try:
                ...
                if has_required_keys:
                    ...
                    if test_case['should_succeed']:
                        results.append((test_case['name'], True, "Context handled properly"))
                    else:
                        results.append((test_case['name'], False, "Should have failed but didn't"))
                else:
...
def test_result_structure_validation():
    ...
    try:
        ...
        # Check required keys
        required_keys = [
            'module_name', 'version', 'success', 'files_processed',
            'entities_replaced', 'warnings_generated'
        ]
        ...
        for key, expected_type in type_checks:
            if not isinstance(result[key], expected_type):
                type_errors.append(f"{key}: expected {expected_type.__name__}, got {type(result[key]).__name__}")
