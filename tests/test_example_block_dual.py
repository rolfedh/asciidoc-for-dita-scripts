...
class DualTestRunner:
    ...
    def _test_deterministic_fixture(self, fixture_name: str):
        ...
        expected_path = self.fixtures_dir / f"{fixture_name.replace('.adoc', '.expected')}"
        
        if not fixture_path.exists():
            self._record_test_result(fixture_name, False, f"Fixture not found: {fixture_path}")
            return
        ...
        try:
            ...
            if fixture_name.startswith('ignore_'):
                if modified_content == content:
                    self._record_test_result(f"{fixture_name} (deterministic)", True, "Correctly ignored")
                else:
                    self._record_test_result(f"{fixture_name} (deterministic)", False, "Should not have been modified")
            else:
                ...
                if expected_path.exists():
                    ...
                    if modified_content.strip() == expected_content.strip():
                        self._record_test_result(f"{fixture_name} (deterministic)", True, "Matches expected output")
                    else:
                        self._record_test_result(f"{fixture_name} (deterministic)", False, "Output doesn't match expected")
                else:
                    # If no expected file, just check that violations were detected
                    if issues:
                        self._record_test_result(f"{fixture_name} (deterministic)", True, f"Detected {len(issues)} violations")
                    else:
                        self._record_test_result(f"{fixture_name} (deterministic)", False, "No violations detected")
                        
        except Exception as e:
            self._record_test_result(f"{fixture_name} (deterministic)", False, f"Error: {str(e)}")
    ...
    def _test_interactive_scenario(self, scenario: Dict[str, Any]):
        ...
        if not fixture_path.exists():
            self._record_test_result(f"{fixture_name} (interactive)", False, f"Fixture not found")
            return
        ...
        try:
            ...
            with mock.patch('builtins.input', side_effect=scenario['user_inputs']):
                with mock.patch('builtins.print', side_effect=lambda *args, **kwargs: None):
            ...
            if modified_content == content:
                ...
                if modified_content == det_content:
                    success = False
                    details.append("Interactive mode produced same result as deterministic mode")
                else:
                    # Different output is good - interactive mode is working
                    details.append(f"Interactive mode produced different output with {len(issues)} issues processed")
            
            if success:
                result_msg = f"✅ {scenario['description']} - Interactive mode working correctly"
            else:
                result_msg = f"❌ {scenario['description']} - {'; '.join(details)}"
            
            self._record_test_result(f"{fixture_name} (interactive)", success, result_msg)
            
        except Exception as e:
            self._record_test_result(f"{fixture_name} (interactive)", False, f"Error: {str(e)}")
    
    def _count_block_movements(self, original_lines: List[str], modified_lines: List[str]) -> int:
    ...
    def _show_summary(self):
        ...
        if self.failed_tests == 0:
            print("🎉 All tests passed! Both deterministic and interactive modes work correctly.")
        else:
            print(f"⚠️  {self.failed_tests} tests failed. Review the output above for details.")
