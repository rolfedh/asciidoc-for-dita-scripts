    ...
    try:
        ...
        for workflow_name in sorted(workflows):
            try:
                ...
                print(
                    f"   Progress: {progress.completed_modules}/{progress.total_modules} modules"
                )
                print(
                    f"   Files: {len(workflow.files_discovered)} discovered, {progress.processed_files} processed"
                )
    ...
    try:
        ...
        try:
            ...
            for resolution in resolutions:
                state_icon = {"ENABLED": "✅", "DISABLED": "❌", "ERROR": "⚠️"}.get(
...
def debug_directory_config():
    ...
    try:
        ...
        try:
            ...
            if config:
                filtered_files = get_filtered_adoc_files(
                    str(current_dir), config, find_adoc_files
                )
                print(f"   🔍 Filtered discovery: {len(filtered_files)} .adoc files")

                if len(filtered_files) != len(basic_files):
                    print(
                        f"   📊 Filtering effect: {len(basic_files) - len(filtered_files)} files filtered out"
                    )
            else:
    ...
    try:
        ...
        if storage_dir.exists():
            ...
            try:
                ...
                if not list(storage_dir.glob("*")):
                    storage_dir.rmdir()
                    (
                        storage_dir.parent.rmdir()
                        if not list(storage_dir.parent.glob("*"))
                        else None
                    )
        ...
        try:
            ...
            if loaded_workflow.directory == workflow.directory:
                ...
                print(
                    f"   ❌ Directory mismatch: {loaded_workflow.directory} != {workflow.directory}"
                )
            ...
            if len(loaded_workflow.modules) == len(workflow.modules):
                ...
                print(
                    f"   ❌ Module count mismatch: {len(loaded_workflow.modules)} != {len(workflow.modules)}"
                )
...
Examples:
    ...
    list_parser = subparsers.add_parser(
        'list', help='List all workflows with detailed information'
    )
    ...
    # sequencing command
    sequencing_parser = subparsers.add_parser(
        'sequencing', help='Test ModuleSequencer integration'
    )

    # directory-config command
    dirconfig_parser = subparsers.add_parser(
        'directory-config', help='Test DirectoryConfig integration'
    )

    # test-storage command
    storage_parser = subparsers.add_parser(
        'test-storage', help='Test storage system and permissions'
    )
