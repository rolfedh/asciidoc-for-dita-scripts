...
def test_phase_1_warning_suppression():
    ...
    try:
        from adt_core.module_sequencer import ModuleSequencer, LEGACY_PLUGINS
        ...
        print(f"   After set_suppress_legacy_warnings(False): {sequencer.suppress_legacy_warnings}")
        
        sequencer.set_suppress_legacy_warnings(True)
        print(f"   After set_suppress_legacy_warnings(True): {sequencer.suppress_legacy_warnings}")
...
def test_phase_2_entity_reference_migration():
    ...
    try:
        from asciidoc_dita_toolkit.asciidoc_dita.plugins.EntityReference import (
            EntityReferenceModule, main, ADT_MODULE_AVAILABLE
        )
...
def test_cli_integration():
    ...
    try:
        ...
        modules_suppressed = get_new_modules_with_warnings_control(suppress_warnings=True)
...
def test_entity_replacement_functionality():
    ...
    try:
        from asciidoc_dita_toolkit.asciidoc_dita.plugins.EntityReference import (
            replace_entities, ENTITY_TO_ASCIIDOC, SUPPORTED_ENTITIES
        )
