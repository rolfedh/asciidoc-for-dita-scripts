...
class TestComprehensiveCoverageGaps:
    ...
    def test_module_crash_during_execution(self):
        ...
        assert (
            "Module crashed!"
            in updated_workflow.modules["DirectoryConfig"].error_message
        )
    ...
    def test_module_initialization_failure(self):
        ...
        failing_module = Mock()
        failing_module.initialize.return_value = {
            "status": "error",
            "message": "Init failed",
        }
        failing_module._initialized = False

        mock_sequencer.available_modules = {"DirectoryConfig": failing_module}
    ...
    def test_directory_config_import_failure_handling(self):
        ...
        workflow = WorkflowState(
            "import_test", str(self.test_project), ["DirectoryConfig"]
        )
    ...
    def test_corrupted_json_recovery(self):
        ...
        workflow = WorkflowState(
            "corrupt_test", str(self.test_project), ["DirectoryConfig"]
        )
    ...
    def test_missing_backup_corruption_handling(self):
        ...
        workflow = WorkflowState(
            "no_backup_test", str(self.test_project), ["DirectoryConfig"]
        )
        state_path = workflow.get_storage_path()
    ...
    def test_atomic_write_interruption_recovery(self):
        ...
        workflow = WorkflowState(
            "atomic_test", str(self.test_project), ["DirectoryConfig"]
        )
        ...
        workflow.mark_module_completed(
            "DirectoryConfig",
            Mock(files_processed=5, files_modified=2, execution_time=1.5),
        )
    ...
    def test_concurrent_workflow_access(self):
        ...
        workflow = WorkflowState(
            ...
            ["DirectoryConfig", "CrossReference"],
        )
        workflow.save_to_disk()
        ...
        def modify_workflow(thread_id):
            ...
            try:
                local_workflow = WorkflowState.load_from_disk(
                    "concurrent_test", self.temp_storage
                )
                ...
                if thread_id == 0:
                    local_workflow.mark_module_completed(
                        "DirectoryConfig",
        ...
        final_workflow = WorkflowState.load_from_disk(
            "concurrent_test", self.temp_storage
        )
    ...
    def test_user_confirmation_yes_response(self):
        ...
        workflow = WorkflowState(
            "confirm_test", str(self.test_project), ["DirectoryConfig"]
        )
        workflow.save_to_disk()
    ...
    def test_user_confirmation_no_response(self):
        ...
        # Mock workflow for cleanup test
        workflow = WorkflowState(
            "no_confirm_test", str(self.test_project), ["DirectoryConfig"]
        )
        workflow.save_to_disk()
    ...
    def test_bulk_cleanup_with_confirmation(self):
        ...
        for i in range(3):
            workflow = WorkflowState(
                f"bulk_test_{i}", str(self.test_project), ["DirectoryConfig"]
            )
            workflow.mark_module_completed(
                "DirectoryConfig",
    ...
    def test_large_file_count_performance(self):
        ...
        for i in range(100):
            (large_project / f"doc_{i:03d}.adoc").write_text(
                f"= Document {i}\nContent {i}"
            )
    ...
    def test_unicode_path_handling(self):
        ...
        workflow = WorkflowState(
            "unicode_тест", str(unicode_project), ["DirectoryConfig"]
        )
        workflow.save_to_disk()
    ...
    def test_very_long_error_messages(self):
        ...
        workflow = WorkflowState(
            "error_test", str(self.test_project), ["DirectoryConfig"]
        )
    ...
    def test_workflow_with_many_modules(self):
        ...
        workflow = WorkflowState(
            "many_modules_test", str(self.test_project), many_modules
        )
    ...
    def test_status_display_formatting_edge_cases(self):
        ...
        workflow.mark_module_failed(
            "CrossReference",
            "Very long error message that might cause formatting issues when displayed in the terminal",
        )
