...
'''
    ...
    def test_wheel_contains_required_modules(self, build_wheel, notify_user):
        ...
        with zipfile.ZipFile(wheel_path, 'r') as wheel:
            ...
            asciidoc_dita_files = [
                f for f in file_list if f.startswith('asciidoc_dita_toolkit/')
            ]
            assert (
                asciidoc_dita_files
            ), "asciidoc_dita_toolkit package not found in wheel"

            # Check for modules package under asciidoc_dita_toolkit (new v2.1.0+ structure)
            modules_files = [
                f for f in file_list if f.startswith('asciidoc_dita_toolkit/modules/')
            ]
            assert (
                modules_files
            ), "asciidoc_dita_toolkit.modules package not found in wheel - refactoring failed!"
            ...
            for expected_file in expected_files:
                ...
                ), f"Critical file {expected_file} missing from wheel"
            ...
            assert (
                'modules' not in top_level_dirs
            ), "Top-level 'modules' directory should be removed after refactoring"
            assert (
                'asciidoc_dita_toolkit' in top_level_dirs
            ), "Top-level 'asciidoc_dita_toolkit' directory missing from wheel"
    ...
    def test_entry_points_defined_correctly(self, build_wheel, notify_user):
        ...
        with zipfile.ZipFile(wheel_path, 'r') as wheel:
            ...
            entry_points_files = [
                f for f in wheel.namelist() if f.endswith('entry_points.txt')
            ]
            ...
            for script in expected_scripts:
                assert (
                    script in entry_points_content
                ), f"Entry point '{script}' not found in metadata"
            ...
            for plugin in expected_plugins:
                assert (
                    plugin in entry_points_content
                ), f"Plugin entry point '{plugin}' not found in metadata"
    ...
    def test_fresh_install_and_import(self, build_wheel, tmp_path, notify_user):
        ...
        notify_user.notify_slow_test_start(
            "Setting up clean environment and testing imports", 12
        )
        wheel_path = build_wheel
        ...
        # Install the wheel
        result = subprocess.run(
            [str(pip_exe), "install", str(wheel_path)], capture_output=True, text=True
        )
        ...
        result = subprocess.run(
            [str(python_exe), "-c", import_test_script], capture_output=True, text=True
        )

        if result.returncode != 0:
            pytest.fail(
                f"Import test failed in clean environment: {result.stderr}\nStdout: {result.stdout}"
            )

        # Verify success messages
        assert "SUCCESS: asciidoc_dita_toolkit imported" in result.stdout
        assert (
            "SUCCESS: asciidoc_dita_toolkit.modules.entity_reference imported"
            in result.stdout
        )
        assert (
            "SUCCESS: asciidoc_dita_toolkit.modules.content_type imported"
            in result.stdout
        )
    ...
    def test_cli_commands_accessible(self, build_wheel, tmp_path, notify_user):
        ...
        for cmd in cli_commands:
            ...
            if cmd_exe.exists():
                ...
                result = subprocess.run(
                    [str(cmd_exe), "--help"], capture_output=True, text=True, timeout=10
                )

                # Command should either succeed or fail gracefully (not with ModuleNotFoundError)
                if result.returncode != 0:
                    assert (
                        "No module named 'modules'" not in result.stderr
    ...
    def test_plugin_discovery_works(self, build_wheel, tmp_path, notify_user):
        ...
        result = subprocess.run(
            [str(python_exe), "-c", plugin_test_script], capture_output=True, text=True
        )

        if result.returncode != 0:
            pytest.fail(
                f"Plugin discovery test failed: {result.stderr}\nStdout: {result.stdout}"
            )
    ...
    def test_version_consistency(self, build_wheel, notify_user):
        ...
        assert (
            pyproject_version in wheel_name
        ), f"Wheel name {wheel_name} doesn't contain version {pyproject_version}"
        ...
        with zipfile.ZipFile(wheel_path, 'r') as wheel:
            ...
            assert (
                f"Version: {pyproject_version}" in metadata_content
    ...
    def test_packaging_config_includes_modules(self):
        ...
        assert (
            'include = ["asciidoc_dita_toolkit*"]' in pyproject_content
        ...
        assert (
            '"modules*"' not in pyproject_content
