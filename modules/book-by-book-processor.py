...
def get_book_files_and_mapping_ruby(master_file):
    """
    Build ID mapping using Ruby + Asciidoctor for proper attribute resolution
    """
    import json
    import tempfile
    
    # Create temporary file for Ruby output
    with tempfile.NamedTemporaryFile(mode='w', suffix='.json', delete=False) as temp_file:
        temp_path = temp_file.name
    
    try:
        # Get the script directory and Ruby script path
        script_dir = os.path.dirname(os.path.abspath(__file__))
        ruby_script = os.path.join(script_dir, 'id-filename-map.rb')
        
        # Run Ruby script to generate ID mapping
        result = subprocess.run(['ruby', ruby_script, master_file, temp_path], 
                              capture_output=True, text=True)
        
        if result.returncode != 0:
            print(f"Ruby script failed: {result.stderr}")
            # Fall back to Python approach
            return get_book_files_and_mapping_python(master_file)
        
        # Read the JSON output
        with open(temp_path, 'r') as f:
            id_to_full_path = json.load(f)
        
        # Convert full paths to just filenames and resolve paths properly
        file_id_dictionary = {}
        included_files = set()
        
        master_dir = os.path.dirname(os.path.abspath(master_file))
        
        for id_val, full_path in id_to_full_path.items():
            filename = os.path.basename(full_path)
            file_id_dictionary[id_val] = filename
            
            # Resolve path relative to master file directory
            if not os.path.isabs(full_path):
                resolved_path = os.path.join(master_dir, full_path)
        ...
        print(f"Error using Ruby approach: {e}")
        # Fall back to Python approach
        return get_book_files_and_mapping_python(master_file)
...
def get_book_files_and_mapping_python(master_file):
    """
    Build ID mapping using Python regex (fallback approach)
...
def get_book_files_and_mapping(master_file):
    ...
    Uses Ruby + Asciidoctor for proper attribute resolution, falls back to Python
    """
    # Check if Ruby is available
    try:
        subprocess.run(['ruby', '--version'], capture_output=True, check=True)
        return get_book_files_and_mapping_ruby(master_file)
    except (subprocess.CalledProcessError, FileNotFoundError):
        print(f"Ruby not available, using Python approach for {master_file}")
        return get_book_files_and_mapping_python(master_file)
...
def main():
    ...
    # Process each book individually (correct approach)
    total_changes = 0
    total_issues = []
    
    for master_file in master_files:
        changes, issues = process_book(
            master_file, 
