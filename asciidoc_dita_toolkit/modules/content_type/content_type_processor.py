...
class ContentTypeProcessor:
    ...
    def ensure_blank_line_after_attribute(
        self, lines: List[Tuple[str, str]], index: int
    ) -> List[Tuple[str, str]]:
        """
    ...
    def update_existing_attribute(
        ...
        logger.debug(
            "Updating existing attribute at line %d: %s -> %s",
            i,
            attribute.value,
            new_value,
        )
        ...
        if attribute.attribute_type == 'deprecated_content':
            ...
            # Uncomment the line
            text = text.replace(
                "//:_mod-docs-content-type:", ":_mod-docs-content-type:", 1
            )
        ...
        current_format_indices = [
            j
            for j, (line_text, _) in enumerate(lines)
            if j != i
            and (
                line_text.strip().startswith(":_mod-docs-content-type:")
                or line_text.strip().startswith("//:_mod-docs-content-type:")
            )
        ]
    ...
    def add_new_attribute(
        self, lines: List[Tuple[str, str]], content_type: str
    ) -> List[Tuple[str, str]]:
        """
    ...
    def _handle_existing_attribute(
        self,
        filepath: str,
        lines: List[Tuple[str, str]],
        ...
        if (
            existing_attribute.attribute_type == 'current'
            and existing_attribute.value.strip()
        ):
            self.ui.show_success(f"File: {filename} — Updated: {content_type}")
        else:
            # Convert from deprecated format
            old_type = (
                existing_attribute.value.strip()
                if existing_attribute.value.strip()
                else "deprecated"
            )
            self.ui.show_success(
                f"File: {filename} — Converted: {old_type} → {content_type}"
            )

        return True

    def _handle_new_attribute(
        self, filepath: str, lines: List[Tuple[str, str]], analysis: dict
    ) -> bool:
