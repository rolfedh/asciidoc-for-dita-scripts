...
class ModuleSequencer:
    ...
    def load_configurations(
        self, dev_config_path: str, user_config_path: str = None
    ) -> None:
    ...
    def discover_modules(self) -> None:
        ...
        try:
            ...
            eps = entry_points()
            adt_eps = (
                eps.select(group="adt.modules")
                if hasattr(eps, 'select')
                else eps.get("adt.modules", [])
            )
            ...
            for entry_point in adt_eps:
                try:
                    ...
                    if not isinstance(module_instance, ADTModule):
                        ...
                        if (
                            self.suppress_legacy_warnings
                            and entry_point.name in LEGACY_PLUGINS
                        ):
                            self.logger.debug(
                                f"Legacy plugin {entry_point.name} does not inherit from ADTModule (transition mode)"
                            )
                        else:
                            self.logger.warning(
                                f"Module {entry_point.name} does not inherit from ADTModule"
                            )
                        continue

                    self.available_modules[module_instance.name] = module_instance
                    self.logger.info(
                        f"Discovered module: {module_instance.name} v{module_instance.version}"
                    )
    ...
    def sequence_modules(
        self, cli_overrides: Optional[Dict[str, bool]] = None
        ...
        try:
            ...
            final_modules = self._apply_user_preferences(sorted_modules, cli_overrides)
            ...
            for dep in all_dependencies:
                if dep not in self.available_modules:
                    raise MissingDependencyError(
                        f"Module '{module_name}' depends on missing module '{dep}'"
                    )
    ...
    def _apply_user_preferences(
        self, sorted_modules: List[str], cli_overrides: Optional[Dict[str, bool]]
    ) -> List[str]:
        ...
        module_requirements = {}
        for module_config in self.dev_config.get("modules", []):
            module_requirements[module_config["name"]] = module_config.get(
                "required", False
            )
        ...
        for module_name in sorted_modules:
            ...
            if cli_override is not None:
                ...
                if module_name in user_disabled:
                    self.logger.warning(
                        f"Ignoring user disable for required module: {module_name}"
                    )
    ...
    def _validate_final_config(
        ...
        for i, module_name in enumerate(enabled_modules):
            ...
            user_overrides = self.user_config.get("moduleOverrides", {}).get(
                module_name, {}
            )
    ...
    def get_module_status(self) -> Dict[str, Any]:
        ...
        for module_name in configured_modules:
            if (
                module_name not in enabled_modules
                and module_name in self.available_modules
            ):
                module = self.available_modules[module_name]
                status["modules"].append(
                    {
...
def determine_module_state(
    ...
    if module_name in cli_overrides:
        return (
            ModuleState.ENABLED if cli_overrides[module_name] else ModuleState.DISABLED
        )
