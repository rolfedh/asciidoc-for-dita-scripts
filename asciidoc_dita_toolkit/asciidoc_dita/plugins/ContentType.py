...
try:
    ...
    raise ImportError(
        ...
        f"This is required for ContentType plugin to function properly."
    )
    ...
    def initialize(self, config: Dict[str, Any]) -> None:
        ...
        if detector_config and isinstance(detector_config, dict):
            ...
            self.detector_config = ContentTypeConfig(
                filename_prefixes=detector_config.get(
                    "filename_prefixes",
                    ContentTypeConfig.get_default().filename_prefixes,
                ),
                title_patterns=detector_config.get(
                    "title_patterns", ContentTypeConfig.get_default().title_patterns
                ),
                content_patterns=detector_config.get(
                    "content_patterns", ContentTypeConfig.get_default().content_patterns
                ),
            )
        else:
...
def create_processor(
    ...
    config: Optional[ContentTypeConfig] = None,
...
def process_content_type_file(
    filepath: str, processor: Optional[ContentTypeProcessor] = None
):
    """
...
def main(args):
    ...
    if ADT_MODULE_AVAILABLE:
        ...
        if (
            not config["batch_mode"]
            and not config["legacy_mode"]
            and not config["quiet_mode"]
        ):
