...
class CrossReferenceProcessor:
    ...
    def __init__(self):
        ...
        self.id_in_regex = re.compile(r'''(?<=['"])[^_"']+''')
        ...
        self.link_in_regex = re.compile(r'(?<=xref:)(?!.*\.adoc#)([^\[]+)(\[.*?\])')
...
def process_master_file(filepath: str) -> None:
    ...
    if not processor.id_map:
        print(Highlighter(f"No IDs found in {filepath} or its includes").warn())
...
def main(args):
    ...
    if hasattr(args, 'master_file') and args.master_file:
        ...
        if not os.path.exists(args.master_file):
            print(Highlighter(f"Error: Master file {args.master_file} not found").warn())
            sys.exit(1)
        ...
        if not master_files:
            print(Highlighter(f"No master.adoc files found in {directory}").warn())
        ...
        if os.path.exists(master_file):
            ...
            print(Highlighter(f"No master.adoc found in {directory}. Use --recursive to search subdirectories or specify --master-file.").warn())
