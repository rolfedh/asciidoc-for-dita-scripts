    ...
    if is_plugin_enabled("DirectoryConfig"):
        try:
            ...
            if config:
                ...
                adoc_files = get_filtered_adoc_files(
                    directory_path, config, find_adoc_files
                )
                if adoc_files:
                    ...
                    logger.info(
                        f"Processing {len(directories)} {dir_text}{exclude_text}"
                    )
                    logger.info(
                        f"Found {len(adoc_files)} .adoc file{'s' if len(adoc_files) != 1 else ''} to process"
                    )
                else:
            ...
            logger.debug(
                "DirectoryConfig plugin not available, using legacy file discovery"
            )
            adoc_files = fallback_to_legacy()
    else:
        # Legacy behavior: process all files in directory
