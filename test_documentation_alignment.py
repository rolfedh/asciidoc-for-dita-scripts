...
def test_adtmodule_interface_documentation():
    ...
    try:
        ...
        required_methods = ['name', 'version', 'dependencies', 'release_status', 
            ...
            if method:
                ...
                if not docstring:
                    ...
                    if quality_score >= 1:
                        ...
                        docstring_quality.append(f"Method {method_name} incomplete documentation")
            else:
...
def test_docstring_style_consistency():
    ...
    try:
        ...
        # Get all methods
        methods = inspect.getmembers(EntityReferenceModule, predicate=inspect.isfunction)
            ...
            if method:
                ...
                for param_name, param in signature.parameters.items():
                    ...
                    if param.annotation == inspect.Parameter.empty:
                        type_hint_issues.append(f"Method {method_name} parameter '{param_name}' missing type hint")
                    else:
                        print(f"  ✓ Method {method_name} parameter '{param_name}': {param.annotation}")
                
                # Check return type hint
                if signature.return_annotation == inspect.Signature.empty:
                    if method_name != 'cleanup':  # cleanup can return None
                        type_hint_issues.append(f"Method {method_name} missing return type hint")
                else:
                    print(f"  ✓ Method {method_name} return type: {signature.return_annotation}")
