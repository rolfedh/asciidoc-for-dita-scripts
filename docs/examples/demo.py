...
def main():
    ...
    print(f"   Developer config: {len(sequencer.dev_config['modules'])} modules configured")
    print(f"   User config: {len(sequencer.user_config.get('disabledModules', []))} modules disabled")
    
    # 4. Get module status
    print("\n📊 Current module status:")
    status = sequencer.get_module_status()
    for module_info in status['modules']:
        state_icon = "✓" if module_info['state'] == "enabled" else "⨯" if module_info['state'] == "disabled" else "?"
        print(f"   {state_icon} {module_info['name']} [{module_info['state']}, v{module_info['version']}]")
    ...
    override_enabled = [r for r in override_resolutions if r.state == ModuleState.ENABLED]
